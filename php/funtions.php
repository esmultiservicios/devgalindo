...
{
	$mes_nombre = '';
	$mes = date('n', strtotime($fecha));  // 'n' devuelve el número del mes sin ceros a la izquierda
	switch ($mes) {
		case 1:
			$mes_nombre = 'enero';
			break;
...
{
	$dia = dia_nombre($fecha);  // Obtiene el nombre del día
	$numero_dia = date('d', strtotime($fecha));  // Día con dos dígitos
	$mes = nombre_mes($fecha);  // Obtiene el nombre del mes
	$anio = date('Y', strtotime($fecha));  // Año con cuatro dígitos

	return $dia . ', ' . $numero_dia . ' de ' . $mes . ' de ' . $anio;
}
