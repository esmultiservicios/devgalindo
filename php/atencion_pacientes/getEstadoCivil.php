...
try {
    ...
    if (!$stmt) {
        throw new Exception('No se pudo preparar la consulta de Estado Civil: ' . $mysqli->error);
    }

    if (!$stmt->execute()) {
        throw new Exception('No se pudo consultar Estado Civil: ' . $stmt->error);
    }
    ...
    if ($resultado->num_rows === 0) {
        ...
        while ($registro = $resultado->fetch_assoc()) {
            $id = (int) $registro['estado_civil_id'];
