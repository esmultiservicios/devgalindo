...
try {
    ...
    if (!$stmt) {
        throw new Exception('No se pudo preparar la consulta de Escolaridad: ' . $mysqli->error);
    }

    if (!$stmt->execute()) {
        throw new Exception('No se pudo consultar Escolaridad: ' . $stmt->error);
    }
    ...
    if ($resultado->num_rows === 0) {
        ...
        while ($registro = $resultado->fetch_assoc()) {
            $id = (int) $registro['escolaridad_id'];
